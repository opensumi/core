...
class TypeScriptReferencesCodeLensProvider extends baseCodeLensProvider_1.TypeScriptBaseCodeLensProvider {
    resolveCodeLens(inputCodeLens, _token) {
        const codeLens = inputCodeLens;
        return vscode.commands.executeCommand('vscode.executeReferenceProvider', codeLens.document, codeLens.range.start).then((locations) => {
            if (!locations) {
                throw codeLens;
            }
