  ...
  private patchTheme() {
    ...
    if (this.settings.filter((setting) => !setting.scope).length === 0) {
      this.settings.unshift({
        settings: {
          foreground: this.colors['editor.foreground'] && this.colors['editor.foreground'].substr(0, 7),
          background: this.colors['editor.background'] && this.colors['editor.background'].substr(0, 7),
        },
      });
    }
    reg.setTheme(this);
