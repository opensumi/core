...
export class RPCStub {
  ...
  private clientServiceProxyMap: Map<string, Promise<RPCProxy>> = new Map();
  ...
  public registerStubClientServiceProxy(servicePath: string, connection: any, stubClientId: string) {
    if (this.clientServiceProxyResolveMap.has(servicePath)) {
  ...
  public getRegisterClientService(servicePath: string) {
    return this.clientServiceProxyMap.get(servicePath);
  }

  public async getClientService(servicePath: string, clientService?: RPCService): Promise<any> {
    if (!this.clientServiceProxyMap.has(servicePath)) {
      ...
      this.clientServiceProxyMap.set(servicePath, service as Promise<RPCProxy>);
    }

    const stubClientServiceProxy = await this.clientServiceProxyMap.get(servicePath) as RPCProxy;
