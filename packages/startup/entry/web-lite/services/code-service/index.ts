...
export class CodeServiceImpl implements ICodeService {
  async fetchContent(repo: string, path: string, ref: string) {
    ...
    const targetUrl =
      process.env.SCM_PLATFORM === 'aone'
        ? `/code-service/v3/projects/${encodeURIComponent(repo)}/repository/files?file_path=${encodeURIComponent(
            path.slice(1),
          )}&ref=${ref}`
        : `/code-service/v4/projects/${encodeURIComponent(repo)}/repository/files/${encodeURIComponent(
            path.slice(1),
          )}?ref=${ref}`;
