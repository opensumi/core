 ...
 * @param fallback 找不到 icon 是否回退到 octicon
 */
export function getCodIcon(iconKey: string, fallback = true) {
  if (fallback && !codIconId.includes(iconKey)) {
    return getOctIcon(iconKey);
  }
  return `codicon codicon-${iconKey}`;
}
