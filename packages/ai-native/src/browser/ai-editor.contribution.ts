...
import { IDisposable, URI, MaybePromise, Disposable, Event, uuid, CommandService } from '@opensumi/ide-core-common';
  ...
  contribute(editor: IEditor): IDisposable {
    ...
    Event.debounce(
      ...
      this.disposables.push(aiContentWidget.onSelectChange(async (value) => {
        ...
        if (value === '解释代码') {
          this.aiChatService.launchChatMessage({ message: '/explain ', prompt: this.aiChatService.explainCodePrompt() });
          return;
        }
