...
**Agent Session Discovery Loading**: The page-local loading presentation shown while an Agent Session Discovery Refresh is in flight and the Agent Session Browser holds no Sessions. It appears as a centered loading indicator in the browser, ends when the refresh settles regardless of Agent query outcomes, and reports no failure detail. _Avoid_: Pending Agent Session Selection, session/load spinner, error state
