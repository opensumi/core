...
**Standby ACP Process**: An initialized ACP Agent process that is not bound to a Task Conversation and is available for a compatible Task Launch or Task Conversation restoration. Maintaining standby capacity is best-effort under finite resource limits and does not guarantee that every ACP configuration can avoid process cold start. _Avoid_: Guaranteed warm session, pre-created Task Conversation, unlimited warm pool

**ACP Standby Target**: The exact ACP runtime configuration selected by the current or last confirmed Task Launch context for which Agentic Layout attempts to maintain Standby ACP Process capacity throughout the IDE page lifecycle. Project Addition and historical Task Selection do not change it; changing the Task Draft's selected ACP Agent or Workspace Target replaces it, so an unbound standby or in-flight warmup for a superseded target may be reclaimed without affecting active Task Conversations. _Avoid_: Global warm pool, any idle Agent process, per-Agent-only target
...
**Agent Task**: A durable unit of Agent work bound to exactly one Workspace Target, exactly one originating ACP Agent, and exactly one Task Conversation. Its ACP Agent binding does not automatically change when another Agent becomes current or the originating Agent becomes unavailable. Selecting a Task restores its Task Conversation in Agentic Layout without changing the IDE's current Workspace; the bound Workspace Target remains visible as the Task's execution context. _Avoid_: Chat, conversation, background job
...
**Durable Agent Task**: An Agent Task whose ACP session state and recoverable history outlive browser reloads, workspace navigation, temporary disconnection, closure of the originating Web IDE page, and replacement of its Web IDE container. Container replacement may restart the originating ACP Agent but must not invalidate the Task Conversation owned by the same ACP Agent Identity; the host platform and ACP Agent provide this durability without Agentic Layout prescribing their storage mechanism. _Avoid_: Persistent chat, detached process
...
**Task Launch**: The Agentic Layout flow that first opens a Task Draft and creates a Durable Agent Task only after its first Prompt has been accepted by ACP and the request stream has been established. Launching from the Agentic Chat View header uses the selected Task's Workspace Target when a Task Conversation is active, otherwise it uses the current IDE Workspace Target; launching from a Project Group uses that group's Workspace Target directly. The selected Workspace Target provides the Agent's working directory without changing the IDE's current Workspace. Its primary fields are task description and ACP Agent; model, permission mode, and other optional parameters are advanced settings. A failed or developer-cancelled Task Launch before that acceptance point preserves the previous Active Task and its unsent draft; it must not leave a late Session activation or Durable Agent Task. _Avoid_: Unbound new chat, projectless task

**Task Draft**: The Agent-bound composition state opened by Task Launch before a Durable Agent Task exists. Its first Prompt may use a temporary ACP session while launching, but the Task Conversation and Agent Task become durable only when ACP accepts that Prompt. _Avoid_: New Session, empty Task
...
**Session-first Task Selection**: The Task List interaction that lazily validates and restores the selected Task Conversation through its originating ACP Agent, including pending interactions and task artifacts, while leaving the IDE's current Workspace, file tree, and editor state unchanged. The Agent Task List does not eagerly start every related Agent to validate all retained sessions. A Task may remain fully interactive when its bound Workspace Target differs from the IDE's current Workspace; the Agentic Chat View must make that target visible as execution context. When selections overlap, only the latest selection may become active or clear a Task's unread marker; superseded loads release their local session references and attachments without stopping Agent work already running. A failed selection preserves the previously active Task Conversation, Task Row, and unread state rather than opening an unbound draft. _Avoid_: Workspace-aware Task Switch, workspace navigation, eager session reconciliation, read-only task replay

**Pending Task Selection**: The temporary selection intent while Agentic Layout prepares a requested Task Conversation. It is represented by a spinner and busy semantics on the requested Task Row without implying ACP Task Status, replacing the current Active Task, or blocking another selection; only the latest Pending Task Selection may replace the Active Task, and a failed or superseded selection never becomes active. _Avoid_: Active Task, loading conversation, selected Task

**Transcript Ready**: The Task Conversation condition in which its retained conversation content can be presented consistently. Transcript Ready is sufficient to replace the previous Agentic Chat View and does not imply that live Agent communication has resumed. _Avoid_: Session loaded, Live Ready, fully connected

**Live Ready**: The independent Task Conversation condition in which new prompts and ACP progress can be exchanged with its originating ACP Agent. A Task Conversation may be Transcript Ready before it is Live Ready; during that interval its transcript and draft remain available, but the draft cannot be submitted, and live restoration must not hide readable content. _Avoid_: Transcript Ready, page ready, history loaded
