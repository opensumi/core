...
**Pending Task Selection**: The temporary selection intent while Agentic Layout prepares a requested Task Conversation. It is represented by the requested Task Row without replacing the current Active Task or blocking another selection; only the latest Pending Task Selection may replace the Active Task, and a failed or superseded selection never becomes active. _Avoid_: Active Task, loading conversation, selected Task
