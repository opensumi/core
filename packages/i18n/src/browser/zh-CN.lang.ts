export const localizationBundle = {
  ...
  contents: {
    ...
    'debug.launch.configurations.debugServer': '仅用于调试扩展开发: 如果已指定端口，VS 代码会尝试连接到在服务器模式中运行的调试适配器',
    ...
    'preference.files.watcherExclude': '配置排除文件监听的文件路径 glob 模式，模式必须在绝对路径上匹配（即前缀为 ** 或完整路径才能正确匹配）。更改此设置需要重新启动。若在启动时遇到代码占用大量 CPU 时间，可以排除大文件夹以减少初始加载。',
    'preference.files.exclude': '配置排除的文件和文件夹的 glob 模式，资源管理器将根据此设置决定要显示或隐藏的文件和文件夹。',
    ...
    'editor.configuration.tabSize': '控制 Tab 缩进等于的空格数。若启用 `#editor.detectIndentation#`，该设置可能会被覆盖',
    ...
    'preference.workbench.list.openMode': '控制如何使用鼠标展开树和打开列表项（若支持）。如果此设置不适用，某些树和列表可能会选择忽略此设置。',
    ...
    'keymaps.keybinding.success': '快捷键修改成功',
