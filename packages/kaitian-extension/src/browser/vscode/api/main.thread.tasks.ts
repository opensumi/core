...
import { ILogger, IDisposable, uuid, formatLocalize, Uri, withNullAsUndefined, ITaskDefinitionRegistry, isString, URI, Disposable } from '@ali/ide-core-common';
import { ITaskService, ITaskProvider, IWorkspaceFolder } from '@ali/ide-task/lib/common';
import { KeyedTaskIdentifier, TaskEventKind, TaskExecution } from '@ali/ide-task/lib/common/task';
import { IWorkspaceService } from '@ali/ide-workspace';
import { PresentationOptions, RunOptions, CommandOptions, CommandConfiguration, RuntimeType, TaskSource, TaskSourceKind, ExtensionTaskSource, TaskScope, Task, ConfiguringTask, CustomTask, ContributedTask } from '@ali/ide-task/lib/common/task';
...
namespace TaskDTO {
  ...
  export function to(task: TaskDTO | undefined, workspace: IWorkspaceService, executeOnly: boolean, context): ContributedTask | undefined {
...
export class MainthreadTasks extends Disposable implements IMainThreadTasks {
  ...
  constructor(private rpcProtocol: IRPCProtocol) {
    ...
    this.context = {
      createTaskIdentifier: (identifier, reporter) => {
        return this.taskDefinitionRegistry.createTaskIdentifier(identifier, reporter);
      },
    };

    this.addDispose(this.taskService.onDidStateChange((event) => {
  ...
  $registerTaskProvider(handler: number, type: string): Promise<void> {
    ...
    const provider: ITaskProvider = {
      ...
      provideTasks: (validTypes: Record<string, boolean>) => {
        ...
        return Promise.resolve(this.proxy.$provideTask(handler, validTypes))
          .then((result) => {
            const tasks: Task[] = [];
  ...
  $executeTask(value: TaskHandleDTO | TaskDTO): Promise<TaskExecutionDTO> {
    ...
    if (TaskHandleDTO.is(value)) {
      ...
      if (workspaceFolder) {
        this.taskService.getTask(workspaceFolder.codeUri, value.id, true).then((task: Task) => {
