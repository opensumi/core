...
describe('kaitian extHostWebview Test', () => {
  ...
  const kaitianExtHostWebview = new KaitianExtHostWebview(rpcProtocol as any);

  it('ext host kaitian webview test', async (done) => {

    const handle1 = await kaitianExtHostWebview.getWebviewHandle('existingWebview');
    expect(mainService.$connectPlainWebview).toBeCalledWith('existingWebview');

    const handle2 = await kaitianExtHostWebview.createPlainWebview('testPlain', 'path/to/icon');
    ...
    kaitianExtHostWebview.$acceptMessage(handle2Id, 'testMessage');
    expect(onMessageHandler).toBeCalledWith('testMessage');
