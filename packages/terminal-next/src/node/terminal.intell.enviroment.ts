...
import { ITerminalIntellEnvironment, ITerminalIntellLogger, Shell, TerminalIntellFileSystem } from '../common/intell/environment';
...
export class TerminalIntellEnviromentNode implements ITerminalIntellEnvironment {
  ...
  buildExecuteShellCommand(timeout: number): Fig.ExecuteCommandFunction {
    return async ({ command, env, args, cwd }: Fig.ExecuteCommandInput): Promise<Fig.ExecuteCommandOutput> => {
      ...
      return new Promise((resolve) => {
        child.on("close", (code) => {
          this.logger.debug({ msg: "shell command done", command, stdout, stderr, code });
          resolve({
            status: code ?? 0,
            stderr,
            stdout,
  ...
  async resolveCwd(cmdToken: CommandToken | undefined, cwd: string, shell: Shell): Promise<{ cwd: string; pathy: boolean; complete: boolean }> {
