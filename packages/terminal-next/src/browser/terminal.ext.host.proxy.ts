...
export class TerminalProcessExtHostProxy extends Disposable implements ITerminalChildProcess, ITerminalProcessExtHostProxy {
  ...
  private readonly _onProcessOverrideDimensions = this.registerDispose(new Emitter<ITerminalDimensionsOverride | undefined>());
  ...
  private readonly _onResize: Emitter<{ cols: number, rows: number }> = this.registerDispose(new Emitter<{ cols: number, rows: number }>());
