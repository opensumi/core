...
export class ApiCommandArgument<V, O = V> {
  ...
  static readonly Position = new ApiCommandArgument<types.Position, IPosition>('position', 'A position in a text document', (v) => types.Position.isPosition(v), typeConverters.fromPosition);
  static readonly Range = new ApiCommandArgument<types.Range, IRange>('range', 'A range in a text document', (v) => types.Range.isRange(v), typeConverters.fromRange);
  static readonly Selection = new ApiCommandArgument<types.Selection, ISelection>('selection', 'A selection in a text document', (v) => types.Selection.isSelection(v), typeConverters.fromSelection);
...
export const newCommands: ApiCommand[] = [
  ...
  new ApiCommand(
    ...
    new ApiCommandResult<[modes.WorkspaceSymbolProvider, modes.IWorkspaceSymbol[]][], types.SymbolInformation[]>('A promise that resolves to an array of SymbolInformation-instances.', (value) => {
  ...
  new ApiCommand(
    ...
    [ApiCommandArgument.Uri, ApiCommandArgument.Position],
    new ApiCommandResult<modes.ICallHierarchyItemDto[], types.CallHierarchyItem[]>('A CallHierarchyItem or undefined', (v) => v.map(typeConverters.CallHierarchyItem.to)),
  ),
  new ApiCommand(
    ...
    new ApiCommandResult<modes.IIncomingCallDto[], types.CallHierarchyIncomingCall[]>('A CallHierarchyItem or undefined', (v) => v.map(typeConverters.CallHierarchyIncomingCall.to)),
  ),
  new ApiCommand(
    'vscode.provideOutgoingCalls', '_executeProvideOutgoingCalls', 'Compute outgoing calls for an item',
    [ApiCommandArgument.CallHierarchyItem],
    new ApiCommandResult<modes.IOutgoingCallDto[], types.CallHierarchyOutgoingCall[]>('A CallHierarchyItem or undefined', (v) => v.map(typeConverters.CallHierarchyOutgoingCall.to)),
  ),
  // --- rename
  new ApiCommand(
    ...
    new ApiCommandResult<modes.IWorkspaceEditDto, types.WorkspaceEdit | undefined>('A promise that resolves to a WorkspaceEdit.', (value) => {
      if (!value) {
        return undefined;
      }
  ...
  new ApiCommand(
    ...
    new ApiCommandResult<modes.CustomCodeAction[], (vscode.CodeAction | vscode.Command | undefined)[] | undefined>('A promise that resolves to an array of Command-instances.', (value, _args, converter) => {
      return tryMapWith<modes.CustomCodeAction, vscode.CodeAction | vscode.Command | undefined>((codeAction) => {
  ...
  new ApiCommand(
    ...
    [ApiCommandArgument.Uri],
    new ApiCommandResult<modes.IRawColorInfo[], vscode.ColorInformation[]>('A promise that resolves to an array of ColorInformation objects.', (result) => {
      if (result) {
