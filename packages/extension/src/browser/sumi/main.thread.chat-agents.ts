...
import { IChatAgentService, IChatProgress } from '@opensumi/ide-ai-native/lib/common';
import { IRPCProtocol } from '@opensumi/ide-connection';

import { ExtHostSumiAPIIdentifier } from '../../common/sumi';
import { IMainThreadChatAgents, IExtHostChatAgents, IExtensionChatAgentMetadata } from '../../common/sumi/chat-agents';
...
export class MainThreadChatAgents implements IMainThreadChatAgents {
  private proxy: IExtHostChatAgents;
  ...
  async $handleProgressChunk(requestId: string, data: IChatProgress): Promise<number | void> {
    this.pendingProgress.get(requestId)?.(data);
  }
