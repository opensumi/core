...
test.describe('ACP Chat Agentic History', () => {
  ...
  test('Task List keeps the Agentic workbench visible, filters ordered Tasks, and restores selection safely', async ({
    ...
    const newerTaskStatus = page.getByTestId(`agentic-task-status-${newerTask.sessionId}`);
    const archiveNewerTask = page.getByTestId(`agentic-task-archive-${newerTask.sessionId}`);
    await search.hover();
    await search.focus();
    await expect(newerTaskStatus).toHaveCSS('opacity', '1');
    await page.getByTestId(`agentic-task-row-${newerTask.sessionId}`).hover();
    await expect(newerTaskStatus).toHaveCSS('opacity', '0');
    ...
    await search.hover();
    await search.focus();
    const archivedTaskStatus = page.getByTestId(`agentic-task-status-${newerTask.sessionId}`);
    await expect(archivedTaskStatus).toHaveCSS('opacity', '1');
    await archivedRow.focus();
    await expect(archivedTaskStatus).toHaveCSS('opacity', '0');
