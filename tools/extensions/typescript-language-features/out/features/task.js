...
class TscTaskProvider {
    ...
    async getTasksForProject(project) {
        ...
        if (this.autoDetect === 'watch' || this.autoDetect === 'on') {
            ...
            const watchTask = new vscode.Task(watchTaskidentifier, project.workspaceFolder || vscode.TaskScope.Workspace, localize('buildAndWatchTscLabel', 'watch - {0}', label), 'tsc', new vscode.ShellExecution(command, ['--watch', ...args]), '$tsc-watch');
