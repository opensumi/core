...
class TypeScriptServerSpawner {
    ...
    getTsServerArgs(configuration, currentVersion, pluginManager) {
        const args = [];
        let cancellationPipeName;
        let tsServerLogFile;
        const apiVersion = currentVersion.version || api_1.default.defaultVersion;
...
class TypeScriptServer extends dispose_1.Disposable {
    constructor(_childProcess, _tsServerLogFile, _cancellationPipeName, _logger, _telemetryReporter, _tracer) {
        super();
    ...
    tryCancelRequest(seq, command) {
        try {
            ...
            if (callback) {
                callback.onSuccess(new typescriptService_1.CancelledResponse(`Cancelled request ${seq} - ${command}`));
            }
        }
    }
    dispatchResponse(response) {
        ...
        if (response.success) {
            ...
            callback.onSuccess(new typescriptService_1.NoContentResponse());
        }
        else {
