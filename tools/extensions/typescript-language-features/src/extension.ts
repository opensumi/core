...
import { Api, getExtensionApi, ITSClient } from './api';
...
const onTSServerClientCreated = new vscode.EventEmitter<ITSClient>();

export function extensionWillActivate(ctx: any) {
	(ctx as any).registerPublicApi({
		onTSServerClientCreated:onTSServerClientCreated.event
	})
}
...
function createLazyClientHost(
	...
	return lazy(() => {
      ...
      onTSServerClientCreated.fire(clientHost.serviceClient)
