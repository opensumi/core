...
const jumpTracker_1 = require("../src/jumps/jumpTracker");
...
suite('Record and navigate jumps', () => {
    ...
    const newJumpTest = options => {
        return newTest(Object.assign({ title: `Can track jumps for keys: ${options.keysPressed.replace(/\n/g, '<CR>')}` }, options));
    };
    suite('Jump Tracker unit tests', () => {
        const jump = (lineNumber, columnNumber, fileName) => new jump_1.Jump({
            editor: null,
            fileName: fileName || 'Untitled',
            position: new position_1.Position(lineNumber, columnNumber),
        });
        ...
        test('Can record jumps between files', () => __awaiter(this, void 0, void 0, function* () {
            const jumpTracker = new jumpTracker_1.JumpTracker();
            ...
            assert.deepEqual(jumpTracker.jumps.map(j => j.fileName), ['file1', 'file2', 'file3'], 'Unexpected jumps found');
        ...
        test('Can handle file jump events sent by vscode in response to recordJumpBack', () => __awaiter(this, void 0, void 0, function* () {
            const jumpTracker = new jumpTracker_1.JumpTracker();
            jumpTracker.handleFileJump(null, file1);
            jumpTracker.handleFileJump(file1, file2);
            jumpTracker.handleFileJump(file2, file3);
            ...
            assert.deepEqual(jumpTracker.jumps.map(j => j.fileName), ['file1', 'file2', 'file3', 'file4'], 'Unexpected jumps found');
        ...
        test('Can record jumps between files after switching files', () => __awaiter(this, void 0, void 0, function* () {
            const jumpTracker = new jumpTracker_1.JumpTracker();
            jumpTracker.handleFileJump(null, file1);
            jumpTracker.handleFileJump(file1, file2);
            jumpTracker.handleFileJump(file2, file3);
            ...
            assert.deepEqual(jumpTracker.jumps.map(j => j.fileName), ['file1', 'file2', 'file3', 'file2'], 'Unexpected jumps found');
            assert.equal(jumpTracker.currentJump, null, 'Unexpected current jump found');
        }));
        test('Can handle jumps to the same file multiple times', () => __awaiter(this, void 0, void 0, function* () {
            const jumpTracker = new jumpTracker_1.JumpTracker();
            jumpTracker.handleFileJump(null, file1);
            jumpTracker.handleFileJump(file1, file2);
            jumpTracker.handleFileJump(file2, file3);
        ...
        test('Can record up to 100 jumps, the fixed length in vanilla Vim', () => __awaiter(this, void 0, void 0, function* () {
            const jumpTracker = new jumpTracker_1.JumpTracker();
            range(102).forEach((iteration) => {
            ...
            assert.deepEqual(jumpTracker.jumps.map(j => j.position.line), range(102).slice(2, 102), "Jump tracker doesn't contain the expected jumps after removing old jumps");
        }));
        test('Can handle text deleted from a file', () => __awaiter(this, void 0, void 0, function* () {
            const jumpTracker = new jumpTracker_1.JumpTracker();
            ...
            assert.deepEqual(jumpTracker.jumps.map(j => [j.position.line, j.position.character, j.fileName]), [
                [0, 0, 'file2'],
                [5, 0, 'file2'],
                [0, 0, 'file1'],
                [3, 0, 'file1'],
            ...
            jumpTracker.handleTextDeleted({ fileName: 'file1' }, new vscode.Range(new vscode.Position(3, 0), new vscode.Position(5, 0)));
            ...
            assert.deepEqual(jumpTracker.jumps.map(j => [j.position.line, j.position.character, j.fileName]), [[0, 0, 'file2'], [5, 0, 'file2'], [0, 0, 'file1'], [3, 5, 'file1'], [4, 0, 'file1']], `Jump tracker doesn't contain the expected jumps after deleting two lines`);
            jumpTracker.handleTextDeleted({ fileName: 'file1' }, new vscode.Range(new vscode.Position(3, 0), new vscode.Position(4, 0)));
            // If that results in multiple jumps on a line, though the duplicate is deleted
            // Preserve the newest jump in that case
            assert.deepEqual(jumpTracker.jumps.map(j => [j.position.line, j.position.character, j.fileName]), [[0, 0, 'file2'], [5, 0, 'file2'], [0, 0, 'file1'], [3, 0, 'file1']], `Jump tracker doesn't contain the expected jumps after deleting another line`);
            jumpTracker.handleTextDeleted({ fileName: 'file1' }, new vscode.Range(new vscode.Position(0, 0), new vscode.Position(3, 0)));
            // If you delete lines such that jumps are past EOF, delete the jumps
            assert.deepEqual(jumpTracker.jumps.map(j => [j.position.line, j.position.character, j.fileName]), [[0, 0, 'file2'], [5, 0, 'file2'], [0, 0, 'file1']], `Jump tracker doesn't contain the expected jumps after deleting all lines in file`);
        }));
    });
...
//# sourceMappingURL=jumpTracker.test.js.map
