...
| `acp-chat-agentic-long-history-switching` | `history --history-message-count=1000` | Two deterministic 1,000-message Agent Sessions, stable per-Session sentinels, rich variable-height rows, and hardened bounded-DOM Playwright coverage exist. | No shared mock-agent fixture gap for the long-history switching pass. |
...
| `acp-chat-agentic-session-archive-and-restore` | `history`, `stream-rich` | Seeded Agent-owned Sessions, stable ordering, rich replay, normal send, and hardened local Archive/Unarchive Playwright coverage exist. | No shared mock-agent fixture gap for the history-backed archive/restore pass. |
| `acp-standby-capacity-and-session-launch` | service harness plus `stream-rich` | Process-backed normal creation exists; deterministic pool capacity, warmup, cancellation, and first-Prompt races are provided by focused service/browser harnesses. | Runtime UI may reuse existing launch/cancel scenarios; no additional shared mock-agent fixture is required for the node-contract pass. |
