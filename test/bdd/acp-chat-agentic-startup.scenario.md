...
9. Use the Agentic header focus action to enter full-screen chat, restore the split view, then record the workbench and Explorer geometry.
...
- With fresh Agentic layout state, AI Chat and the editor/workbench are both rendered as the default split view.
- The status bar, editor/workbench, and Explorer remain visible by default.
- The Agentic focus action can temporarily hide the editor/workbench and Explorer, and the restore action returns to the split view without changing the workspace URL or active ACP session.
- In the default and restored split view, AI Chat is the leftmost major column within `640px <= AI Chat width <= 1440px`, and the workbench width is at least `480px`.
...
- **PASS** - Agentic AI Chat starts beside the visible workbench, focus mode remains reversible, default ACP Chat tools are safe and canonical, and state responses are metadata-only.
- **BLOCKED** - Common Preflight, the MCP bridge, or the Agentic layout launch profile is unavailable.
- **FAIL** - fresh Agentic layout hides the workbench, focus mode cannot be restored to the split view, Agentic layout is unusable, tool names drift, old mutation tools are exposed, or state responses leak content.
